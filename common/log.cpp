...
#define TIMESTAMP_FORMAT_STRING " {:4.4f} "
...
{
	std::string tag;
	tag += fmt::format("[{} |", "L"); // TODO: Get Log Level as string
	tag += fmt::format(TIMESTAMP_FORMAT_STRING, GetCurrentMessageTime());
	tag += fmt::format("| {}] ", "CHAN"); // TODO: Get Channel as string
	return tag;
}
...
std::string Log::GetChannelNameString()
{
	return std::string();
}
...
void Log::Write(LogChannels channel, LOGLEVEL level, ConsoleColors color, std::string_view message)
{
...
void Log::Writef(LogChannels channel, LOGLEVEL level, ConsoleColors color, const char* format, ...)
{
	std::va_list ap;
	va_start(ap, format);
